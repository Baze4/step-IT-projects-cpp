...
void save(stud *S)
{
	...
	f=fopen("student 192.step","w+b");
	
	fprintf(f," %d\n",N);
	
	for(int i=0;i<N;i++)
	{
		fprintf(f,"%s %s %d %f %d\n",S[i].nume,S[i].grupa,S[i].virsta,S[i].media,S[i].NRnote);
...
void load(stud *S)
{
	...
	FILE *f;   f=fopen("student 192.txt","r");
	
	fscanf(f,"%d",&N);
	
	for(int i=0;i<N;i++)
	{
		fscanf(f,"%s %s %d %f %d",&S[i].nume,&S[i].grupa,&S[i].virsta,&S[i].media,&S[i].NRnote);
